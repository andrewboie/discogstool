...
class APIBase(object):
    ...
    @property
    def _response(self):
        print "Fetching", self._uri, self._params
        return requests.get(self._uri, params=self._params, headers=self._headers)
...
class Release(APIBase):
    ...
    @property
    def artist_str(self):
        return u" ".join([unicode(i) for i in self.artists])

    @property
    def label_str(self):
        return u" ".join([unicode(i) for i in self.labels])

    @property
    def format_str(self):
        fmts = []
        for f in self.data.get("formats"):
    ...
    def tsv(self):
        ...
        if rls == "unknown":
            ...
            mm = self.released[5:7]
            if not mm:
                mm = "00"
            dd = self.released[8:10]
            if not dd:
                dd = "00"
    ...
    def __unicode__(self):
        return (u'%s - "%s" [%s, %s] (%s)' %
                (self.artist_str, self.title, self.label_str,
                 self.released, self.format_str))
